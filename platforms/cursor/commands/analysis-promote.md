...
Run the existing Quick-to-Full succession logic from `/analysis-new --from {ID}`:
...
1. **Quick file**: Update Status to `Promoted to Full: {new Full ID}`
2. **status.md**:
   - Change Quick row's Stage to `Promoted -> {new Full ID}`
